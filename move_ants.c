...
void move_ants(t_lem_in *lem_in)
{
	...
	while (lem_in->end->ants_here != lem_in->num_ants)
	{
		...
		while (ant_start)
		{
			if (ant_start->path->next && (ant_start->path->next->room == lem_in->end ||
				ant_start->path->next->room->ants_here == 0))
			{
				ft_printf("L%u-%s ", ant_start->num, ant_start->path->next->room->name);
